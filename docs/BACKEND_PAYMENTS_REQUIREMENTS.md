...
| Qué pasa si un límite atado a tier queda excedido tras una baja automática (ej. `TEAM_MEMBER_LIMITS`, `store/team-store.js`) | Ej. entrenador premium con 50 miembros baja a `base` (límite 10) | Sin una regla del backend (¿bloquear agregar nuevos sin tocar a los existentes? ¿forzar reducción?), el frontend no sabe qué mostrar en ese estado |
...
| Necesidad | Detalle | Por qué |
|---|---|---|
| `mp-connect`: `GET /api/v1/mercadopago/connect`, `GET /api/v1/mercadopago/connect/callback`, `GET /api/v1/mercadopago/connect/status` | Ya descriptos en el documento de propuesta backend original | Vinculación OAuth del entrenador, condición previa para poder crear cualquier preferencia con split |
...
| Fecha de vencimiento/próxima renovación por membership de equipo | No existe hoy | Necesaria para mostrar "vence en 3 días" y decidir cuándo mostrar urgencia de renovación al corredor |
| Job programado de expulsión automática si no se renueva dentro del plazo de gracia | Reutiliza la misma acción de dominio que la expulsión manual ya existente (`RunnerActionsMenu`) | Confirmado por el usuario: pago manual, válido 1 mes, plazo de gracia, expulsión automática si no se renueva |
| **Precio mínimo de mensualidad por equipo** (piso configurable por Paceron) | El entrenador fija el precio, pero no puede ser menor a un mínimo — confirmado por el usuario, motivo explícito: evitar que el entrenador cobre por fuera en efectivo para eludir la comisión | Frontend necesita consultar el mínimo vigente y validar contra él en la pantalla de configuración de precio del entrenador (nueva, sin ubicación definida todavía) |
| Endpoint/campo para que el entrenador configure el precio de su equipo | No existe ningún endpoint de esto hoy | Superficie de UI completamente nueva, no contemplada en ningún doc hasta esta ronda |
...
| Semántica exacta de `vencido`/`en_prueba` | ¿Hay período de gracia visible como estado propio o es parte de `vencido`? ¿Existe `en_prueba` en la práctica? | El frontend no debe inferir estos criterios localmente |
| Efecto de "salir del equipo" (ya existe) sobre el cobro asociado | ¿Corta en el momento o deja correr hasta el cierre del período ya pagado? | Sin definir todavía, ver checklist en decisiones de B |
...
1. **Backend puede arrancar Sub-proyecto A ya** — es lo más spec-eado y menos ambiguo de los dos, el documento de propuesta original ya lo cubre casi por completo (salvo `status_detail` y el vencimiento/renovación de tier, marcados arriba).
2. **Sub-proyecto B puede arrancar en paralelo, no está bloqueado por arquitectura** — la decisión de fondo (recurrencia simulada + split real) ya está tomada. Lo que sí conviene resolver antes de comprometerse a un contrato final: los tres valores numéricos (comisión, precio mínimo, plazo de gracia) y la pantalla de configuración de precio del entrenador, que es superficie nueva no contemplada hasta ahora.
