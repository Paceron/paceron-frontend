...
| `GET` | `/users/{id}/next-session` | — | **Pendiente (Gap 10), shape propuesto:** `200 {next_cancelled, next_training}`, ambos nullable de forma independiente, cada uno `{group_id, group_name, date, session_name, is_presencial?, presencial_time_from?, presencial_time_to?, presencial_location?}` (`is_presencial`/`presencial_*` solo en `next_training`, no aplica a `next_cancelled`) — el más próximo cronológicamente de cada kind (`cancelled`/`training`) entre TODOS los grupos de los que es miembro, `date >= hoy` (mismo criterio de "día cerrado" del corredor: hoy cuenta si el presencial todavía no arrancó). Para el banner de "próximo entrenamiento" del home del corredor — ver Gap 10 |
| `GET` | `/users/{id}/next-presencial-session` | — | **Pendiente (Gap 10):** `200 {group_id, group_name, team_id, team_name, date, session_name, presencial_time_from, presencial_time_to, presencial_location}` \| `204` si ninguno de los grupos que ADMINISTRA (`owner_id`) tiene una próxima `GroupCalendarDay` con `kind='training'`, `is_presencial=true` y `date >= hoy` (no cerrada) — la primera cronológicamente entre todos sus grupos administrados, sin importar equipo. Para el banner de "próxima sesión presencial" del home del entrenador |
...
| `GET` | `/users/{id}/member-calendar?from={date}&to={date}` | — | **Pendiente (Gap 11):** array de `GroupCalendarDay` de TODOS los grupos de los que es miembro en el rango, cada item con `group_id`/`group_name`/`team_id`/`team_name` sumados. Vista agregada de calendario del corredor (pieza 2) |
| `GET` | `/users/{id}/administered-calendar?from={date}&to={date}` | — | **Pendiente (Gap 11):** mismo shape, de TODOS los grupos que administra (`owner_id`). Cada día presencial viene marcado con `presencial_collision` (`same_team`/`cross_team` + lista de conflictos) si colisiona con otro grupo administrado en la misma fecha — incluye colisiones same-team (permitidas) y viejas de antes del guard de Gap 9. Vista agregada de calendario del entrenador (pieza 2) |
