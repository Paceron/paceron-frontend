...
| `POST` | `/groups/{id}/calendar/bulk` | `{dates: [...], kind, session_id?, other_name?, is_presencial?, presencial_time?, presencial_location?}` | `200` — multi-select bulk-assign, mismo `kind`/contenido a todas las fechas listadas, misma validación de §3.1 por cada una |
| `POST` | `/groups/{id}/calendar/bulk-clear` | `{dates: [...]}` | `204` — multi-select bulk-clear, borra las filas de esas fechas |
| `POST` | `/groups/{id}/calendar/shift` | `{from_date, days}` | `200` — todas las filas con `date >= from_date` pasan a `date + days`. `days` entero positivo, elegido por el entrenador (no fijo a 1). `409` si el corrimiento haría chocar dos fechas existentes (no debería pasar corriendo hacia adelante, pero se valida igual) |
...
3. Al confirmar, `PUT /sessions/{id}` recibe un campo adicional opcional `{..., exclude_group_ids?: [...]}`.
   - Si `exclude_group_ids` viene vacío u omitido: `PUT` normal, todos los grupos ven el cambio (comportamiento ya descripto en el doc de catálogo).
   - Si trae ids: el backend, **antes** de aplicar el update,
     a. clona la sesión (`POST /sessions/{id}/clone` internamente, mismo criterio de sufijo `" (copia)"` — el frontend puede mandar `clone_name`/`clone_description` opcionales en el mismo body para permitir editarlos en el momento, si no vienen se usa el sufijo default),
     ...
     c. recién ahí aplica el `PUT` con los campos nuevos a la sesión original — la comparten los grupos que quedaron tildados (los no listados en `exclude_group_ids`) más el catálogo en general (nuevas asignaciones futuras).
