...
| `POST` | `/groups/{id}/calendar/stamp` | `{plan_id, start_date, force?, exclude_dates?}` | `201 {days: [...], same_team_warnings: [...]}` (Gap 9, wrapper — antes era array crudo) — copia cada `PlanDay` del plan a partir de `start_date` (día 1 del plan → `start_date`, día 2 → `start_date + 1`, etc.), incluyendo `default_presencial`/`default_time_from`/`default_time_to`/`default_location` si los tiene. Si algún día del rango ya tiene contenido y `force` no es `true`, `409` con la lista de fechas en conflicto (sin tocar `exclude_dates`). Colisión presencial cross-equipo (Gap 9) → `409 {message, conflicts}`, sin `force` que la salve; same-team → se guarda, `same_team_warnings` en la respuesta |
| `POST` | `/groups/{id}/calendar/bulk` | `{dates: [...], kind, session_id?, other_name?, is_presencial?, presencial_time_from?, presencial_time_to?, presencial_location?}` | `200 {days: [...], same_team_warnings: [...]}` (Gap 9, wrapper) — multi-select bulk-assign, mismo `kind`/contenido a todas las fechas listadas, misma validación de §3.1 por cada una. All-or-nothing: colisión cross-equipo en cualquier fecha → `409 {message, conflicts}` con TODAS las fechas conflictivas, no se escribe nada |
| `POST` | `/groups/{id}/calendar/bulk-clear` | `{dates: [...]}` | `204` — multi-select bulk-clear, borra las filas de esas fechas |
| `POST` | `/groups/{id}/calendar/shift` | `{from_date, days}` | `200 {days: [...], same_team_warnings: [...]}` (Gap 9, wrapper) — todas las filas con `date >= from_date` pasan a `date + days`. `days` entero positivo, elegido por el entrenador (no fijo a 1). All-or-nothing: `409` si el corrimiento choca dos fechas existentes, o si alguna fecha nueva colisiona en presencial con otro equipo (`{message, conflicts}`) |
| `GET` | `/users/{id}/next-session` | — | `200 {next_cancelled, next_training}` (Gap 10, resuelto), ambos nullable de forma independiente, cada uno `{group_id, group_name, date, session_name}` — `next_training` suma `is_presencial, presencial_time_from?, presencial_time_to?, presencial_location?` (`{lat, lng, label?}`). El más próximo cronológicamente de cada `kind` (`cancelled`/`training`) entre TODOS los grupos de los que es miembro, `date >= hoy` (hoy cuenta si el presencial todavía no arrancó). Para el banner de "próximo entrenamiento" del home del corredor |
| `GET` | `/users/{id}/next-presencial-session` | — | `200 {group_id, group_name, team_id, team_name, date, session_name, presencial_time_from, presencial_time_to, presencial_location}` (Gap 10, resuelto) \| `204` si ninguno de los grupos que ADMINISTRA (`owner_id`) tiene una próxima `GroupCalendarDay` con `kind='training'`, `is_presencial=true` y `date >= hoy` (no cerrada) — la primera cronológicamente entre todos sus grupos administrados, sin importar equipo. Para el banner de "próxima sesión presencial" del home del entrenador |
...
| `GET` | `/users/{id}/member-calendar?from={date}&to={date}` | — | `200` array de `GroupCalendarDay` (Gap 11, resuelto) de TODOS los grupos de los que es miembro en el rango, cada item con `group_id`/`group_name`/`team_id`/`team_name` sumados. `400` si falta `from`/`to`, formato inválido, o `from > to`. Vista agregada de calendario del corredor (pieza 2) |
| `GET` | `/users/{id}/administered-calendar?from={date}&to={date}` | — | `200` mismo shape (Gap 11, resuelto), de TODOS los grupos que administra (`owner_id`). Cada día presencial que colisiona trae `presencial_collision: {type: 'same_team'\|'cross_team', conflicts: [...]}` (`cross_team` gana si hay de ambos) — incluye colisiones same-team (permitidas) y viejas de antes del guard de Gap 9; ausente si no colisiona. Misma validación de `from`/`to` que `member-calendar`. Vista agregada de calendario del entrenador (pieza 2) |
...
**Colisión presencial cross-grupo (Gap 9, `BACKEND_API_GAPS.md`, RESUELTO 2026-09-22):** `PUT`/`stamp`/`bulk`/`shift` rechazan con `409 {message, conflicts}` sin `force` posible cuando un día que queda `is_presencial=true` colisiona en horario con un día presencial de un grupo de OTRO equipo del mismo `owner_id` (overlap medio-abierto — bordes que se tocan no colisionan; `cancelled` nunca participa). Colisión con un grupo del MISMO equipo no bloquea — se guarda y suma `same_team_warnings` (mismo shape que `conflicts`) a la respuesta. Ver tabla de arriba para el shape exacto por endpoint (rompió el shape de respuesta de `stamp`/`bulk`/`shift`, ahora wrapeado).
