...
**Actualización 2026-09-21 (ronda 2), feedback de uso real + Gap 7 resuelto:**
- **`TimeField` "desde"/"hasta" ahora van en una sola fila** (`flex-row`, cada uno `flex-1`) en vez de apilados — entran cómodos en ambas plataformas, no hacía falta reservarlo para desktop-only (a diferencia de `Row`/`Col`, que solo pasa a fila en web ancho).
- **Celdas del mes con tinte de fondo leve por `kind`** (mismo color que el punto, con alpha bajo — `26` normal, `14` si el día está cerrado) — antes solo el punto/pin distinguían el tipo de día, ahora se nota de un vistazo sin tener que fijarse en un detalle chico.
- **Gap 7 resuelto** (`docs/BACKEND_API_GAPS.md`) — ver §6 para el detalle: el select de sesión ahora ofrece "Mantener sesión actual" (`KEEP_CURRENT_SESSION`) cuando el día ya tiene instancia, en vez del match-por-nombre que se había armado como mejor esfuerzo (`utils/session-instance-match.js`, eliminado). `session_instance.sourceSessionId`/`exercises[].sourceExerciseId` (origen de catálogo, informativo) se suman al modelo.
- **Limpieza: "asignar un plan al momento de crear/editar un grupo" retirado por completo** — selector de plan sacado de `create-group-modal.jsx`/`edit-group-screen.jsx`/`group-list-editor.jsx`/`team-detail-screen.jsx`, `TRAINING_PLAN_OPTIONS`/`trainingPlanId` sacados de `store/team-store.js`/`services/normalizers.js#toGroupModel`. Se retiró además toda la pantalla `assign-training-plan-screen.jsx` (ruta `/training-plans/[planId]/assign`, botón "Asignar" en `training-plan-detail-screen.jsx`) — asignaba un plan a un grupo o a un corredor individual por el mismo mecanismo 100% local ya descartado; el mecanismo real para grupos es este calendario, y la asignación individual a corredor queda pendiente de reescritura recién cuando se aborde la pieza 3 (vista del corredor). `store/training-plan-store.js` perdió `groupTrainingPlanIds`/`assignToGroup`/`assignToRunner`/`unassignFromRunner` — `fetchMyPlans` quedó simplificado a solo asignación individual.
...
- ~~No hay forma exacta de saber de qué sesión de catálogo salió una instancia ya creada~~ — **Gap 7 resuelto 2026-09-21**, ver actualización más abajo.
...
2. Campos condicionales:
   ...
   - Entrenamiento → `ResponsiveSelectField` con las sesiones del catálogo del entrenador (mismo `useSessions(ownerId)` ya existente). Si el día ya tiene una instancia asignada (`existingDay.sessionInstance`): se muestra un bloque de solo lectura "Sesión asignada actualmente" (nombre + descripción congelados) arriba del select, y el select se preselecciona en una opción extra al inicio de la lista, "Mantener sesión actual (sin cambios)" (`KEEP_CURRENT_SESSION`, `services/normalizers.js`) — dejarla así hace que `toCalendarDayPayload` no mande `session_id` en el `PUT`, y el backend conserva la instancia sin reinstanciar (Gap 7, resuelto 2026-09-21). Elegir cualquier sesión real del catálogo reemplaza el contenido por una instancia nueva. Si el día es nuevo (sin instancia previa), el select no tiene esa opción y es obligatorio elegir una sesión. + toggle "¿Es presencial?".
...
- ~~Evitar la reinstanciación en guardados que no tocan la sesión~~ — **hecho 2026-09-21** (Gap 7 resuelto, `KEEP_CURRENT_SESSION`, ver §6).
