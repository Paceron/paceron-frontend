...
Confirmado por el usuario: cada tier, por rol, habilita **funciones dedicadas** — no es un badge sin efecto funcional. Hoy el único ejemplo concreto que existe en el código es `TEAM_MEMBER_LIMITS` (`store/team-store.js`, límite de miembros por equipo según el tier del entrenador). **No hay todavía un catálogo completo de qué desbloquea cada nivel más allá de ese caso** — a medida que se agreguen otras funciones gateadas por tier, deberían aparecer en `GET /api/v1/tiers` (junto con precio/moneda) para que el frontend las liste en `tier-upgrade-screen.jsx` como parte del argumento de venta de cada nivel, no solo mostrar el precio.
...
- **Si no se renueva dentro del plazo de gracia (5 días, decidido), el tier baja automáticamente a `base`** — mismo tipo de mecanismo que la expulsión automática de equipo en Sub-proyecto B (lo dispara el backend, el frontend solo refleja el resultado). Mismo valor que el plazo de gracia de B (misma mecánica, se decidió unificarlo).
...
- **Bloqueo amplio, no solo "no podés sumar gente":** al entrar al equipo (entrenador o corredor), se muestra una pantalla/aviso indicando que hace falta recuperar el tier o reducir la cantidad de miembros para poder seguir gestionándolo/usándolo con normalidad. **Para el corredor: modo solo lectura** — decidido así aunque hoy no exista ninguna función paga concreta que cortarle (el plan de entrenamiento, la función más obvia a restringir, todavía no está implementado — `TRAINING_PLAN_OPTIONS = []` en `store/team-store.js`). Se deja la restricción definida a nivel de UI desde ahora para no tener que rediseñarla cuando existan funciones de corredor atadas a tier.
