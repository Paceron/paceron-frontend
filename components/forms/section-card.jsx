...
// backend todavía).
export function SectionCard({ title, icon, children, collapsible = false, collapsed = false, onToggle, variant = 'default', headerRight = null, scope = 'section-card' }) {
  ...
  return (
    <View className={`mb-5 rounded-2xl border p-6 shadow-sm ${style.box}`} nativeID={scope} testID={scope}>
      <View className={`flex-row items-center gap-2 ${!collapsed || !collapsible ? 'mb-4' : ''}`} nativeID={`${scope}-header-row`} testID={`${scope}-header-row`}>
        <Header
          className={`flex-1 flex-row items-center gap-2 ${collapsible ? 'hover:opacity-70 active:opacity-70' : ''}`}
          nativeID={`${scope}-header`}
          onPress={collapsible ? onToggle : undefined}
          testID={`${scope}-header`}
        >
          {collapsible && (
            <Animated.View nativeID={`${scope}-chevron`} style={chevronStyle} testID={`${scope}-chevron`}>
          ...
          <Text className={`text-base font-bold ${style.title}`} nativeID={`${scope}-title`} testID={`${scope}-title`}>{title}</Text>
        </Header>
        {headerRight}
      </View>
