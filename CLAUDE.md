...
**El ícono de rol que no abre el menú probablemente NO es un problema de arbitraje de gestos:** el usuario agregó un `console.log` en el handler del botón y confirmó que SÍ se imprime al tocar — el `onPress` llega. Esto contradice la hipótesis de las notas anteriores (tap swallowed por el Pan ancestro). Sospecha más probable, sin confirmar todavía: `AnimatedDropdown` (overlay `position: absolute`, `z-50`) renderizando detrás de otro elemento por stacking context — la fila reordenable tiene su propio `zIndex` animado (`rowStyle`, 0 o 10 según si está activa), y el propio `Modal` puede introducir su propio contexto de stacking. A investigar por separado de cualquier tema de gestos.
