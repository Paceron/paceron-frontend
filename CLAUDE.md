...
**Drag-and-drop:** todo el DnD del catálogo (`components/plans/session-drag-and-drop.jsx` — arrastrar del catálogo de ejercicios a la lista de una sesión, indicador de posición, autoscroll) es **casero**, sobre `react-native-gesture-handler`/`react-native-reanimated` directo, sin librería de terceros. **`react-native-drax` se evaluó y se descartó** (2026-09-13/14, rama `feature/catalog-redesign`): se adoptó brevemente para reordenar una lista por arrastre con handle (`DraxList`/`DraxHandle`), pero el gesto nunca se activó con mouse real (2 pruebas) y coincidió con un crash en Expo Go — la librería (v1.1.0, reescrita para Reanimated 4 y **Gesture Handler 3**) declara `react-native-gesture-handler` como peer dep mínima `>=2.0.0`, pero en la práctica necesita v3; este proyecto está en v2.28. Revertido a botones arriba/abajo para reordenar (sin drag), ya probados. No reintentar sin antes confirmar gesture-handler v3 instalado y funcionando en el resto de la app.

**Reordenamiento por mantener-presionado en mobile/narrow (2026-09-14):** para esa plataforma sí hay reordenamiento por drag (`ReorderableRow`/`ReorderProvider` en `session-drag-and-drop.jsx`) — la clave que evitó el problema de gesture-handler v3 de arriba es `Gesture.Pan().activateAfterLongPress(ms)` (API nativa de gesture-handler v2, no una librería nueva): el Pan no se activa hasta pasado el hold, así que un scroll normal (vertical en la lista de la sesión, horizontal en la tira de catálogo) no compite por el mismo gesto. Mismo mecanismo reusado para el cross-container en la tira horizontal del catálogo (`DraggableExerciseCard` con prop `holdMs`). El layout ancho (desktop) sigue con botones arriba/abajo sin cambios — el hold solo hizo falta donde la fila/card vive dentro de un contenedor scrolleable en la misma dirección del drag. Matemática de índices (estimar posición, clampear, reordenar array) vive en `session-reorder-math.js`, sin dependencia de reanimated/gesture-handler a propósito — es lo único de este mecanismo testeable con Jest (`__tests__/session-reorder-math.test.js`); el gesto en sí (timing del hold, arbitraje contra el scroll nativo) no se puede simular de forma confiable con eventos de puntero sintéticos en el preview — mismo límite ya documentado más arriba, verificación real queda para dispositivo/browser real.

**Gestos dentro de un `Modal` necesitan su propio `GestureHandlerRootView` (2026-09-14):** `react-native`'s `Modal` monta su contenido en una superficie nativa separada del árbol de vistas de la app — el `GestureHandlerRootView` que envuelve toda la app (`app/_layout.jsx`) no alcanza ahí en nativo. Sin uno propio adentro del `Modal`, cualquier gesto (`GestureDetector`, `ScrollView`/`FlatList` de gesture-handler) queda mudo en nativo sin ningún error — bug real en `CreateSessionModal`: ni el scroll ni el hold-and-drag respondían en Expo Go pese a que la estructura y el código de los gestos estaban bien. En web nunca se notó porque el `Modal` de `react-native-web` es una `View` con estilos, no una superficie nativa aparte. Regla general: todo `Modal` que use gesture-handler (`GestureDetector`, o los `ScrollView`/`FlatList` re-exportados de esa librería) necesita un `<GestureHandlerRootView style={{flex: 1}}>` propio envolviendo su contenido — ver `CreateSessionModal` como referencia.

**Hold-to-drag dentro de un `ScrollView`/`FlatList` de gesture-handler — checklist (2026-09-14, tras el fix de arriba):** con el root ya resuelto, quedaron 3 problemas más finos, los 3 con el mismo tipo de causa (arbitraje de gestos, no bugs de lógica):
- **Scroll bloqueado al lado de un `GestureDetector` con `activateAfterLongPress`:** sin más config, un swipe rápido (sin hold) queda atrapado por el Pan sin activarse y sin soltar el toque a tiempo para que el `ScrollView` lo tome — el scroll queda muerto, no solo lento. Fix: agregar `.failOffsetX([-10, 10])` (lista horizontal) o `.failOffsetY([-10, 10])` (lista vertical) al Pan — si el dedo se mueve más de eso antes de cumplirse el hold, el gesto falla al instante y libera el toque. No afecta el hold-and-drag ya andando (solo cambia el comportamiento ANTES de que se arme).
- **Controles anidados (`Pressable`, selects) dejan de responder al tap dentro de una fila con `GestureDetector`:** el Pan se queda con el toque incluso para un tap corto. Fix: `Gesture.Simultaneous(pan, Gesture.Native())` en vez de pasar el Pan solo al `GestureDetector` — dejar correr un `Gesture.Native()` en paralelo es lo que gesture-handler ofrece específicamente para que los controles nativos de adentro sigan funcionando normal.
- **Warning "None of the callbacks in the gesture are worklets":** cosmético (los callbacks corren bien igual, en JS thread), pero se saca agregando `.runOnJS(true)` al gesto — declara la intención explícita en vez de dejar que gesture-handler lo infiera y avise.

Los 3 fixes viven en `session-drag-and-drop.jsx` (`DraggableExerciseCard`, `ReorderableRow`). Igual que con el fix del root, esto **no se pudo verificar con eventos de puntero sintéticos en el preview** (el timing de fallo/activación es justamente lo que la simulación no puede replicar de forma confiable) — quedó pendiente de confirmación en dispositivo real al escribir esta nota.

**Actualización 2026-09-14, confirmado en Expo Go:** con lo de arriba, el reordenamiento por hold-and-drag y el scroll VERTICAL de la lista de ejercicios de la sesión ya funcionan. Dos cosas seguían rotas, con causas más finas todavía dentro de la misma familia de problema (arbitraje de gestos):

- **Scroll horizontal de la tira de catálogo, seguía sin funcionar pese a `failOffsetX`:** el `failOffsetX` solo no alcanza — hace falta ADEMÁS `Gesture.Simultaneous(pan, Gesture.Native())` en el propio `DraggableExerciseCard` (antes solo tenía el Pan solo), igual que ya se había hecho en `ReorderableRow`. La diferencia entre "vertical ya andaba" y "horizontal seguía roto" era exactamente esa: a `ReorderableRow` sí se le había agregado el `Simultaneous(pan, Native())`, a `DraggableExerciseCard` no. Aplicado solo cuando hay `holdMs` (la tira) — el panel ancho de escritorio (activación inmediata, sin `ScrollView` compitiendo) no lo necesita y no se tocó.
- **Controles anidados adentro de una fila reordenable seguían sin responder al tap** (ícono de rol, confirmado roto por el usuario) pese al `Simultaneous(pan, Native())` a nivel de LA FILA: ese Native() a nivel de fila resolvió el arbitraje con el `ScrollView` hermano (por eso el scroll vertical ya andaba), pero NO alcanza para que un `Pressable` anidado varios niveles adentro reciba el tap — son dos problemas de arbitraje distintos. Fix: envolver cada control interactivo puntual (`SessionRoleClosedSelect`, botón de quitar, el wrapper del select de ejercicio) con su PROPIO `<GestureDetector gesture={Gesture.Native()}>`, no alcanza con ponerlo una sola vez arriba en la fila.

Moraleja parcial (corregida más abajo): `Gesture.Native()` sí resuelve el arbitraje "esta fila vs. su contenedor scrolleable" — pero el envoltorio individual por control (`GestureDetector gesture={Gesture.Native()}` puntual sobre `SessionRoleClosedSelect`, el botón de quitar, el select de ejercicio) **no resolvió el tap anidado** — confirmado roto por el usuario una tercera vez pese a este fix. Reemplazado por el fix estructural de abajo.

**Actualización 2026-09-14 (segunda ronda) — un intento de fix revirtió el propio arrastre, ya vuelto atrás:**

Se probó sacar el Pan de `ReorderableRow` a una capa invisible hermana (`position: absolute, inset 0`) del contenido real, en vez de envolverlo como ancestro — la idea era que el sistema de touch nativo le diera el toque a un `Pressable` real dibujado encima (z-order) sin competir por ningún gesto, evitando el arbitraje en vez de resolverlo. **Esto rompió el propio hold-and-drag por completo, en web Y en mobile** (confirmado por el usuario) — revertido el mismo día a la versión anterior (un solo `GestureDetector` envolviendo el contenido, `Gesture.Simultaneous(pan, Gesture.Native())`, sin capa separada). No se investigó a fondo la causa exacta de la ruptura antes de revertir — prioridad fue restaurar lo que andaba en vez de seguir iterando a ciegas sobre una hipótesis ya débil. Si se retoma esta idea a futuro, hacerlo en un branch/experimento aislado, verificable paso a paso, no como parche directo sobre el mecanismo confirmado funcionando.

Lo que SÍ quedó confirmado y no se tocó en la reversión:
- El hold subido de 300ms a 450ms (`ReorderableRow` y el `holdMs` de la tira de catálogo en `session-exercise-panel.jsx`) — motivo: con 300ms, un swipe LENTO deliberado podía tardar más en cruzar el umbral de `failOffsetX/Y` (10px) que en llegar al hold, activando el Pan como arrastre antes de fallar por movimiento y tragándose el scroll lento. Más margen de tiempo ayuda, pero **el usuario reportó que el scroll horizontal de la tira sigue necesitando un gesto brusco** — este hallazgo por sí solo no alcanzó, sigue abierto.
- El reordenamiento unificado en ambos layouts (ancho ahora usa `ReorderableRow` en vez de botones arriba/abajo, `handleMoveExercise` eliminado) — la unificación en sí no es la causa de la ruptura (la ruptura estaba en `ReorderableRow` mismo, ya revertida), pero como el reordenamiento entero volvió a fallar tras el intento de arriba, esto tampoco quedó confirmado funcionando todavía en ninguno de los dos layouts al momento de escribir esta nota.

**El ícono de rol que no abría el menú: confirmado que NO era arbitraje de gestos — era `AnimatedDropdown` mal anclado (2026-09-14).** El `console.log` del usuario en el handler del botón confirmó que el `onPress` sí llegaba — la hipótesis de gestos (tap swallowed por el Pan ancestro) estaba descartada de entrada. Causa real: **`AnimatedDropdown` (`components/shared/animated-dropdown.jsx`) se posiciona con `top`/`left` relativos al ANCESTRO POSICIONADO MÁS CERCANO, no a la pantalla** — pero `measureInWindow` (usado para calcular esos `top`/`left`) da coordenadas de PANTALLA. Mientras el ancestro más cercano esté cerca del origen de la pantalla (uso típico: dropdowns de header/shell, montados cerca de la raíz), el desfasaje no se nota. Acá el trigger (`SessionRoleClosedSelect`) estaba anidado muy profundo — dentro de `ReorderableRow` (con su propio `transform` animado) dentro de una fila dentro de un `Modal` — así que las coordenadas de pantalla, aplicadas tal cual, no correspondían a dónde el panel realmente terminaba renderizado (fuera de vista o detrás de otro contenido).

**Patrón correcto (ya usado en `exercises-catalog-tab.jsx`, `ExerciseMenuButton`/`containerRef`), aplicado acá:** `AnimatedDropdown` se monta **una sola vez**, arriba (en `CreateSessionModal`, no por fila), anclado al mismo contenedor de referencia (`cardRef`, con `position: 'relative'` explícito) que cada fila usa para medirse. Cada trigger mide DOS cosas con `measureInWindow` — el contenedor de referencia (`containerX/Y`) y su propia posición (`x/y`) — y resta (`x - containerX`, `y - containerY`) para obtener coordenadas relativas a ese contenedor, no a la pantalla. El estado de "qué fila tiene el menú abierto" vive en el componente que monta el `AnimatedDropdown` (acá `CreateSessionModal`, vía `roleMenu = {localKey, anchor}`), no en cada fila — el trigger (`SessionRoleClosedSelect`) quedó sin estado propio, solo mide y reporta hacia arriba (`onOpenMenu`).

Regla general para cualquier `AnimatedDropdown` futuro anidado dentro de una lista/fila (no un header/shell cerca de la raíz): NUNCA montarlo por-fila con `measureInWindow` crudo — medir contra un contenedor de referencia estable y restar, montar el dropdown una sola vez en ese nivel. (Actualización 2026-09-15: el selector de rol de `SessionRoleClosedSelect` se terminó reemplazando por `ResponsiveSelectField` — ver sección "Selects" — pero el patrón de arriba sigue siendo la referencia correcta para cualquier otro `AnimatedDropdown` anidado que no encaje en un select estándar.)

**El hold-to-drag (`activateAfterLongPress`) solo hace falta en nativo, nunca en web (2026-09-15).** En touch, el mismo dedo que arrastra es el que scrollea — de ahí la necesidad de esperar un hold para distinguir una intención de la otra (ver checklist más arriba). En web esa ambigüedad no existe: la rueda del mouse/trackpad scrollea vía eventos `wheel`, que ni pasan por el `Pan` de gesture-handler (solo reacciona a `pointerdown`/`pointermove`) — así que ahí un click-y-arrastrar puede activarse de inmediato, como cualquier drag-and-drop de escritorio (Trello, Notion, etc. tampoco piden mantener presionado con mouse). Manteníamos `activateAfterLongPress`/`failOffset*` fijo para ambas plataformas por simplicidad, pero en web eso no aportaba nada y encima perdía sensibilidad (un scroll lento con mouse podía perderse contra el umbral de fail, igual que en touch). Patrón actual en `DraggableExerciseCard`/`ReorderableRow` (`session-drag-and-drop.jsx`): `let pan = Gesture.Pan()...; if (!isWeb) pan = pan.activateAfterLongPress(ms).failOffsetX/Y([-10,10]); pan = pan.onStart(...)...` — el hold y el fail-offset se agregan condicionalmente, el resto de la cadena (onStart/onUpdate/onEnd) queda igual en ambas plataformas. Aplica a cualquier gesto de arrastre futuro que conviva con un `ScrollView` (el calendario de asignaciones, cuando se cablee, va a necesitar lo mismo).

**Actualización 2026-09-15 (ronda 1) — `failOffset*` + `Gesture.Native()` genérico NO alcanzan en nativo real; se probó `.simultaneousWithExternalGesture(scrollViewRef)`, tampoco alcanzó sola.** Con el hold + `failOffset*` de arriba, y un `Gesture.Simultaneous(pan, Gesture.Native())` sin nada concreto a lo que apuntar, el scroll (horizontal de la tira y vertical de la lista de ejercicios) seguía sin funcionar bien en mobile real — solo un gesto brusco lograba scrollear alguna vez, o si el toque arrancaba sobre un control nativo (select/botón). Primer intento: reemplazar el `Gesture.Native()` genérico por una relación EXPLÍCITA — un `ref` (`useRef(null)`, colgado del `ScrollView` vía `ref={miRef}`) pasado a la card/fila, y `pan.simultaneousWithExternalGesture(miRef)` (gesture-handler v2, acepta un ref a componente directo). **Diagnóstico posterior (probado en dispositivo, con el hold subido a 5000ms a propósito para descartar timing): esto tampoco alcanzó** — con 5 segundos de margen el scroll seguía fallando en el fondo de la fila, confirmando que NO era una carrera contra el timer (si lo fuera, 5s de margen lo habría arreglado) sino que la relación con el `ScrollView` simplemente no resuelve el problema cuando el `Pan` envuelve directamente el contenido interactivo como ancestro.

**Actualización 2026-09-15 (ronda 2) — causa real: un solo `GestureDetector` envolviendo TODO (fondo + controles) hace que el scroll dependa de dónde arranca el toque.** Pistas del usuario que lo confirmaron: (a) tocar un select o el botón de borrar "casi" dejaba scrollear, tocar el fondo de la fila no — un control nativo (`Pressable`/`PickerField`, sistema de touch de React Native, no gesture-handler) esquiva nuestro `Pan` de entrada porque gana el hit-test por estar dibujado encima, así que ESE toque nunca llega a competir con el `Pan` en absoluto; el fondo de la fila, sin nada encima, sí cae directo en el `Pan` y queda atrapado. (b) Con el teclado en pantalla (modal más chico) todo funcionaba perfecto en cualquier punto — pista no resuelta del todo, probablemente un efecto secundario de que CUALQUIER evento de layout fuerte destrabe algo, no se persiguió más porque atacar la causa real (un solo `GestureDetector` para todo) no depende de entender esa pista.

**Fix real: separar la superficie de arrastre del contenido, igual que un intento anterior del mismo día que se había revertido — pero esta vez midiendo el tamaño con `onLayout` en vez de inferirlo con Yoga.** El intento anterior (mismo 2026-09-14, commit revertido en `b2556a4`) ponía la capa invisible con `position: absolute, top/left/right/bottom: 0` esperando que Yoga la auto-dimensionara copiando el tamaño de su hermano (el contenido real, en flujo normal) — pero eso rompió el arrastre por completo, en web Y en mobile, sin que se investigara la causa exacta antes de revertir (prioridad en ese momento fue restaurar lo que andaba). La hipótesis para el segundo intento: no confiar en que Yoga resuelva bien esa auto-medición entre hermanos en un solo pase — en cambio, `onLayout` sobre el contenido real mide el alto/ancho de verdad (`ReorderableRow`: alto vía `rowHeight` en estado; `DraggableExerciseCard` cuando tiene `holdMs`: alto y ancho vía `cardSize`), y ese número medido se aplica explícito (`height: rowHeight`, `width/height: cardSize.width/height`) a la capa de arrastre — nada que Yoga tenga que inferir. Con esto, cualquier `Pressable`/select real vive en un árbol SIN ningún `GestureDetector` ancestro (la capa de arrastre es hermana, no ancestro), así que el fondo de la fila/card también queda libre para el `ScrollView`, no solo los controles. `simultaneousWithExternalGesture` de la ronda 1 se mantiene (sigue siendo necesario para que la CAPA DE ARRASTRE en sí, cuando SÍ se activa un drag real, no bloquee el scroll de otras filas mientras tanto). **Sin confirmar todavía en dispositivo real al momento de escribir esta nota** (mismo límite de siempre — no se puede probar el timing real del gesto con eventos sintéticos en el preview, y esta vez además la sesión de login no persistió para ni siquiera llegar a la pantalla en el preview web). Si esto vuelve a fallar, el próximo paso sería instrumentar (un `console.log`/contador en `onScroll` del `ScrollView` para confirmar si el evento llega siquiera durante un intento de scroll "fallido") antes de seguir cambiando configuración de gestos a ciegas.

**Estado de aplicación vs. estado de servidor:** `@tanstack/react-query` está instalado y el `QueryClientProvider` ya está armado (`providers/app-providers.jsx`). **Convención:** Zustand para estado de aplicación (sesión, UI, preferencias locales — `store/auth-store.js` para sesión, y lo poco que queda en `store/team-store.js`), TanStack Query para estado de servidor (cualquier dato que se pide/muta contra el backend). Dominios ya migrados a Query: roster de equipo (`hooks/use-team-roster.js`, primer uso real, 2026-08-02), búsqueda de equipos (`hooks/use-team-search.js`), solicitudes de ingreso (`hooks/use-join-requests.js`), suscripción/tier (`hooks/use-tier-subscription.js`), y — desde 2026-09-09, rama `feature/team-store-query-migration-spec` — equipos/grupos/invitaciones (`hooks/use-teams.js`/`hooks/use-groups.js`/`hooks/use-invitations.js`, reemplazando las 17 acciones CRUD que antes vivían en `store/team-store.js`; ver `docs/superpowers/specs/2026-09-09-team-store-tanstack-query-migration-design.md`), perfil de usuario (`hooks/use-user.js`, desde 2026-09-09, rama `feature/auth-store-query-migration-spec` — sesión (token/refreshToken/activeRole/userId) queda en `store/auth-store.js`, `services/api.js` la lee sincrónica fuera de React en cada request; ver `docs/superpowers/specs/2026-09-09-auth-store-tanstack-query-migration-design.md`), y catálogo de ejercicios/sesiones del entrenador (`hooks/use-exercises.js`/`hooks/use-sessions.js`, desde 2026-09-09, rama `feature/exercise-session-store-query-migration-spec` — ver `docs/superpowers/specs/2026-09-09-exercise-session-store-tanstack-query-migration-design.md`). `store/team-store.js` quedó solo con `setGroupTrainingPlan` (100% local, sin endpoint real todavía — gap 4 de `docs/BACKEND_API_GAPS.md`) y los selectores/constantes client-side (`selectAdministeredTeams`, `TEAM_MEMBER_LIMITS`, etc.). Sub-proyecto futuro y último de este esfuerzo, sin fecha: migrar `training-plan-store.js` (su diseño todavía puede cambiar).
...
- **`hooks/use-unsaved-changes-guard.js`** — `useUnsavedChangesGuard(isDirty)` → `{ confirmVisible, guardedClose, confirmDiscard, cancelDiscard, bypassGuard }`. Intercepta back nativo/gesto/header vía `usePreventRemove` (de `@react-navigation/native`, ya viene con `expo-router` — no agregar `@react-navigation/core` como dependencia directa) y expone `guardedClose(closeFn)` para botones de back explícitos y para el backdrop de modales (ver sección "Modales" arriba). Incluye un flag interno (`bypassing`) que desarma el guard un render antes de ejecutar la acción confirmada — sin esto, `usePreventRemove` reintercepta su propia navegación redisparada y hace falta confirmar el descarte dos veces (bug real, corregido en el mismo branch).
- **Navegar después de un submit exitoso → `bypassGuard(action)`, no la navegación a mano.** El form sigue "sucio" después de guardar (los valores no se resetean, no hay razón si nos vamos de la pantalla) — un `router.replace(...)`/`router.back()` bien colocado después del submit igual dispara `usePreventRemove`, que lo intercepta como si fuera una salida sin confirmar y muestra el modal de descarte pese a que el usuario ya guardó (bug real, encontrado en `create-team-screen.jsx` — confirmar la creación mostraba "¿salir sin guardar?"). `bypassGuard` usa el mismo mecanismo interno que `confirmDiscard` pero sin mostrar el modal. Aplica a todo screen con guard que navega tras guardar con éxito — ver `create-team-screen.jsx`, `edit-team-screen.jsx`, `edit-group-screen.jsx`, `edit-profile-screen.jsx`, `create-training-plan-screen.jsx`, `edit-training-plan-screen.jsx`, `assign-training-plan-screen.jsx`. No aplica a los dos modales (`create-session-modal.jsx`/`create-exercise-modal.jsx`, cierran vía prop `visible`/callback, no navegación de ruta) ni a pantallas sin auto-navegación post-submit (`invite-team-members-screen.jsx`).
...
- Los archivos `.md` se pueden importar como string desde el código (ej. el contrato en `data/legal/terms-and-conditions.md`), gracias a `metro-markdown-transformer.js` enganchado en `metro.config.js`. **Cambiar config de Metro no es hot-reloadable:** después de bajar una rama que la toque, arrancar una vez con `npx expo start -c`. El renderer (`components/legal/markdown-view.jsx`) soporta solo un subset de markdown — el alcance exacto está documentado en el header de `utils/markdown-parser.js`. `metro-markdown-transformer.js` importa `@expo/metro-config/build/babel-transformer.js` directo — por eso `@expo/metro-config` está pineado como `devDependency` explícita en `package.json` (mismo valor que `expo` trae como dependencia transitiva). Sin esto, `npm install` puede dejar de hoistearlo a top-level si dos paquetes piden rangos de versión distintos, y `npm test` rompe con "Cannot find module" (bug real, 2026-09-09, disparado por `npx expo install --fix`) — si vuelve a pasar tras alinear versiones, resincronizar el pin al valor que reporte `cat node_modules/expo/package.json | grep '"@expo/metro-config"'`.
...
- **`measureLayout` (API legacy de medición) no es confiable en Android bajo New Architecture (Fabric, `newArchEnabled: true` en `app.config.js`)** — puede fallar en silencio (el callback `onFail` no siempre dispara, así que el tap simplemente no hace nada visible, sin error en consola) cuando se mide contra un ancestro lejano a través de un `ScrollView`. Alternativa Fabric-safe: `measureInWindow` en el nodo y en el contenedor de referencia, restando las coordenadas de ambos para obtener la posición relativa (patrón usado en `RunnerMenu` de `components/team/team-detail-screen.jsx` — el menú de tres puntitos del roster, que con `measureLayout` no abría nada en mobile nativo). Preferir `measureInWindow` con este patrón de resta para cualquier posicionamiento absoluto (menús, tooltips) medido contra un ancestro que no sea el padre inmediato.
- **IDs numéricos en bodies de request al backend real:** `use-team-roster.js` normaliza `userId` a **string** (`String(user_id)`) porque se usa en comparaciones de string en otros lugares del código — pero el backend rechaza bodies JSON con `user_id` no numérico ("cuerpo de solicitud inválido"). Bug real en `addGroupUser` (`services/groups.js`), invisible contra mocks (no validan tipos). Regla: cualquier ID que salga del roster normalizado y vaya a un body de request real necesita `Number(id)` en el service, no asumir que el tipo del roster sirve tal cual para el payload.
- **Invalidar el roster de equipo (TanStack Query) fuera del `try/catch`, no solo en el camino feliz.** `handleConfirmLeaveGroup` en `team-detail-screen.jsx` corre `invalidateRoster()` incondicionalmente después del `try/catch` del request — si la mutación falla a mitad (ej. falla el segundo de dos requests encadenados: sacar del grupo viejo + reasignar al default) el cache queda desincronizado con el estado real del backend y un ícono de acción puede desaparecer/quedar mal sin que haya forma de refrescar salvo salir y volver a entrar a la pantalla. Mismo criterio para cualquier mutación de roster con más de un request encadenado.
- **App version del build nativo (`app.config.js`) toma `version` de `package.json`** (no un valor hardcodeado) — evita un segundo lugar donde bumpear a mano. Corregido 2026-09-07: el campo había quedado fijo en `1.0.0` desde el bootstrap inicial, sin sincronizarse nunca con el versionado incremental real (todo build local `eas build --local` generaba APKs con metadata de versión errónea, aunque el nombre de archivo sí llevara la versión correcta vía `$npm_package_version`).
