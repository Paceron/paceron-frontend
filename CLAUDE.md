...
**Actualización 2026-09-15 — `failOffset*` + `Gesture.Native()` genérico NO alcanzan en nativo real; hace falta `.simultaneousWithExternalGesture(scrollViewRef)`.** Confirmado en dispositivo: con el hold + `failOffset*` de arriba, y un `Gesture.Simultaneous(pan, Gesture.Native())` sin nada concreto a lo que apuntar, el scroll (horizontal de la tira y vertical de la lista de ejercicios) seguía sin funcionar bien en mobile — solo un gesto brusco lograba scrollear alguna vez. Causa: `Gesture.Native()` genérico no establece ninguna relación real con el `ScrollView` específico que envuelve la card/fila — sin una relación explícita, gesture-handler no tiene ningún handler nativo concreto con el que negociar, y el `ScrollView` queda esperando pasivamente a que el `Pan` falle en vez de reconocer el toque en paralelo desde el primer frame. Fix: pasar un `ref` (`useRef(null)`, colgado del `ScrollView` vía `ref={miRef}`) hacia la card/fila, y llamar `pan.simultaneousWithExternalGesture(miRef)` (API de gesture-handler v2, acepta directo un ref a un componente, no hace falta `.withRef()` ni relación legacy) — con esto el `ScrollView` sí reconoce el toque en simultáneo de verdad. Reemplaza al `Gesture.Simultaneous(pan, Gesture.Native())` que había antes en ambos componentes (`DraggableExerciseCard` recibe `scrollViewRef` además de `holdMs`; `ReorderableRow` recibe `scrollViewRef` y usa el mismo `autoScrollRef` que ya existía para el autoscroll). El tap en controles anidados (selector de rol, botón de quitar) NO depende de este mecanismo — eso se resolvió aparte, de raíz, con la capa de arrastre invisible hermana del contenido real en `ReorderableRow` (ver más arriba, "sibling-overlay"). **Sin confirmar todavía en dispositivo real al momento de escribir esta nota** — mismo límite de siempre, no se puede probar el timing real del gesto con eventos sintéticos en el preview.
