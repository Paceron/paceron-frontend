...
**Drag-and-drop:** `react-native-drax` (desde 2026-09-13, rama `feature/catalog-redesign`) para reordenar una lista por arrastre con handle — reemplaza los hand-rolled primitives iniciales del modal de sesiones (`components/plans/session-drag-and-drop.jsx`) para ESE caso puntual, vía `DraxList` + `itemDraxViewProps={{dragHandle:true}}` + `DraxHandle`. El drag **entre dos contenedores distintos** (catálogo → lista de la sesión, y a futuro plan → calendario) sigue siendo el mecanismo casero (`SessionDragProvider`/`DraggableExerciseCard`/`DragGhost`) — el mecanismo equivalente de drax para eso (`useSortableBoard`/`SortableBoardContainer`) está marcado experimental por sus propios autores incluso en la v1.1.0 (reescritura completa para Reanimated 4), decisión explícita de no adoptarlo todavía. Si en algún momento se evalúa de nuevo, no confundir "cross-container experimental" con la API básica de `DraxView` (`payload`/`onReceiveDragDrop`), que sí es estable — la confusión entre ambas costó una ronda completa de investigación.
