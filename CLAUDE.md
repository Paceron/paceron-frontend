...
**Actualización 2026-09-14 (segunda ronda) — fix real del tap anidado, más 2 hallazgos nuevos:**

- **El tap anidado no se arregla con más `Gesture.Native()`, se evita estructuralmente:** en vez de que el `GestureDetector` del Pan envuelva TODO el contenido de la fila (controles incluidos) como ancestro, `ReorderableRow` ahora renderiza dos HERMANOS dentro de un contenedor `position: relative` — una capa invisible `position: absolute, inset 0` con el `GestureDetector`/Pan (la superficie de arrastre), y el contenido real (con sus `Pressable`s) como hermano normal, en flujo, ENCIMA en z-order. El sistema de touch nativo entrega el toque a lo que esté más arriba en ese punto de pantalla — un botón real dibujado encima gana ahí sin competir por ningún gesto; el resto de la fila (sin nada encima) cae a la capa de abajo. Evita el problema de arbitraje ancestro-vs-descendiente en vez de intentar resolverlo con composición de gestos, que en la práctica no funcionó para este caso puntual (si bien sigue haciendo falta para el caso fila-vs-`ScrollView`, ver arriba). Con este cambio, los 3 `GestureDetector`+`Native()` individuales que se habían agregado a los controles anidados se revirtieron por innecesarios.
- **Scroll lento (no solo rápido) seguía costando, tanto horizontal como vertical:** causa distinta a las de arriba — carrera entre `activateAfterLongPress(300)` y `failOffsetX/Y`. Un swipe RÁPIDO cruza el umbral de fail (10px) en pocos milisegundos, mucho antes de los 300ms del hold, así que falla a tiempo y libera el scroll. Un swipe LENTO deliberado puede tardar más en cruzar esos mismos 10px que en llegar a los 300ms — el Pan se activa como arrastre ANTES de llegar a fallar por movimiento, tragándose el scroll lento. Fix: subir el hold de 300ms a 450ms (`ReorderableRow` y el `holdMs` de la tira de catálogo en `session-exercise-panel.jsx`) — da más margen de tiempo para que el movimiento cruce el umbral de fail antes de que el hold se cumpla, sea cual sea la velocidad del gesto.
- **Reordenamiento por hold-and-drag unificado en ambos layouts (2026-09-14):** a pedido explícito del usuario, el layout ancho de escritorio pasó de botones arriba/abajo a `ReorderableRow`, igual que mobile/narrow — `handleMoveExercise` y los botones se eliminaron por completo (dead code). El selector de rol SÍ sigue diferenciado por layout: segmentado (3 íconos) en ancho, donde sobra espacio; cerrado (`SessionRoleClosedSelect`) en mobile/narrow, donde no.
