...
export async function listExercises({ ownerId } = {}) {
  if (USE_MOCKS || FORCE_MOCKS) return await mockListExercises({ ownerId });
...
export async function getExercise(exerciseId) {
  if (USE_MOCKS || FORCE_MOCKS) return await mockGetExercise(exerciseId);
  return await api.get(`/exercises/${exerciseId}`);
}

// POST /api/v1/exercises.
export async function createExercise(payload) {
  if (USE_MOCKS || FORCE_MOCKS) return await mockCreateExercise(payload);
...
export async function updateExercise(exerciseId, updates) {
  if (USE_MOCKS || FORCE_MOCKS) return await mockUpdateExercise(exerciseId, updates);
